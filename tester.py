...
def proc_args():
    ...
    if cfg.dataset == 'casia':
        ...
        test_dataset = CasiaFASD(root,data_partion,cfg.devel_batch_size,for_train=False,shape=shape)
        ...
        test_dataset = ReplayAttack(root,data_partion,cfg.devel_batch_size,for_train=False,shape=shape)
    elif cfg.dataset == 'rose':
        root = '/media/meysam/464C8BC94C8BB26B/ROSE-YOUTU/'
        test_dataset = RoseYoutu(root,data_partion,cfg.devel_batch_size,for_train=False,shape=shape)
    elif cfg.dataset == 'siw':
        root = '/media/meysam/901292F51292E010/SiW/SiW_release/'
        test_dataset = SiW(root,data_partion,cfg.devel_batch_size,for_train=False,shape=shape)
    else:
...
def main():
    ...
    FAR,FRR,threshold = calcEER(lbl,pred,epoch,path,plot=True,precision=0.01)
    HTER = calcHTER(lbl,pred,cfg.threshold)
